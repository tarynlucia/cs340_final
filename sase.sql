...
-- 
-- 
CREATE FUNCTION GetLittlesByOfficer (@OfficerID INT)
RETURNS TABLE 
AS 
RETURN (
    SELECT DISTINCT m.OSU_ID, m.Member_name 
    FROM Members m 
    INNER JOIN Attends a ON m.OSU_ID = a.OSU_ID 
    INNER JOIN Leads l ON a.Event_number = l.Event_number
    WHERE l.OSU_ID = @OfficerID
);
